function save()
{
	localStorage['billStateShort'] = $("#billState").children("option:selected").attr("data-code"); 	
	localStorage['newStateShort'] = $("#newState").children("option:selected").attr("data-code"); 
	//alert(localStorage['billStateShort']);
	localStorage['keywords'] = $("#keywords").val().replace(/(.)\s(.)/g, '$1|$2');
	localStorage['keywords2'] = $("#keywords2").val();
	localStorage['quantity']=$("#quantity").val();
	localStorage['captchaSolver']=$("#captchaSolver").val();
	...
	localStorage['ckey'] = $("#ckey").val();
	//store pick up
	localStorage['storeFirstName'] = $("#storeFirstName").val();
    localStorage['storeLastName'] = $("#storeLastName").val();
    localStorage['storeEmail'] = $("#storeEmail").val();
    localStorage['storePhone'] = $("#storePhone").val();
   //Billing Address    
    ...
    localStorage['giftCard'] = $("#giftCard").val();
    localStorage['giftCardNumber'] = $("#giftCardNumber").val();
    localStorage['giftCardPassword'] = $("#giftCardPassword").val();
...
$(function(){
	try{
		$.ajax({
		  type: "GET",
		  url: "https://mostadvancedbot.com/products/walmart-bot"
		}).done(function( res ) {
    ...
    setStates();
	if (localStorage.getItem("storeFirstName") === null) {
		$("#storeFirstName").val();
	}else{
		$("#storeFirstName").val(localStorage['storeFirstName']);
	}
	if (localStorage.getItem("storeLastName") === null) {
		$("#storeLastName").val();
	}else{
		$("#storeLastName").val(localStorage['storeLastName']);
	}
	if (localStorage.getItem("storeEmail") === null) {
		$("#storeEmail").val();
	}else{
		$("#storeEmail").val(localStorage['storeEmail']);
	}
	if (localStorage.getItem("storePhone") === null) {
		$("#storePhone").val();
	}else{
		$("#storePhone").val(localStorage['storePhone']);
	}
	...
	if (localStorage.getItem("captchaSolver") === null) {
		$("#captchaSolver").val();
	}else{
		$("#captchaSolver").val(localStorage['captchaSolver']);
	}
    if (localStorage.getItem("ckey") === null) {
		$("#ckey").val();
	}else{
		$("#ckey").val(localStorage['ckey']);
	}
	...
	if (localStorage.getItem("giftCardNumber") === null) {
		$("#giftCardNumber").val();
	}else{
		$("#giftCardNumber").val(localStorage['giftCardNumber']);
	}
	if (localStorage.getItem("giftCard") === null) {
		$("#giftCard").val();
	}else{
		$("#giftCard").val(localStorage['giftCard']);
	}
    if (localStorage.getItem("giftCardPassword") === null) {
		$("#giftCardPassword").val();
	}else{
		$("#giftCardPassword").val(localStorage['giftCardPassword']);
	}
	...
	$("#storePickupDetail").hide();
	$("#storePickup").click(function(){
		$("#storePickupDetail").toggle();
	});
	if($("#giftCard").val()=="yes")
	{
		$("#giftCardDiv").show();		  		
	...
	$("#captchaDiv").hide();
	if($("#captchaSolver").val()==="yes"){
		$("#captchaDiv").show();
	}else{
		$("#captchaDiv").hide();
	}			
    $("#captchaSolver").click(function() {
        if($("#captchaSolver").val()==="yes"){
            $("#captchaDiv").show();
        }else{
			$("#captchaDiv").hide();
...
var statesCA='<option role="menu" value="AB" aria-label="Alberta">Alberta</option><option role="menu" value="BC" aria-label="British Columbia">British Columbia</option><option role="menu" value="MB" aria-label="Manitoba">Manitoba</option><option role="menu" value="NB" aria-label="New Brunswick">New Brunswick</option><option role="menu" value="NL" aria-label="Newfoundland and Labrador">Newfoundland and Labrador</option><option role="menu" value="NT" aria-label="Northwest Territories">Northwest Territories</option><option role="menu" value="NS" aria-label="Nova Scotia">Nova Scotia</option><option role="menu" value="NU" aria-label="Nunavut">Nunavut</option><option role="menu" value="ON" aria-label="Ontario">Ontario</option><option role="menu" value="PE" aria-label="Prince Edward Island">Prince Edward Island</option><option role="menu" value="QC" aria-label="Quebec">Quebec</option><option role="menu" value="SK" aria-label="Saskatchewan">Saskatchewan</option><option role="menu" value="YT" aria-label="Yukon">Yukon</option>';
...
function setStates()
{
	...
	switch(state)
	{
		case "US": $("#billState").html(statesUS); 	break;
		case "CA": $("#billState").html(statesCA); 	break;
		case "AU": $("#billState").html(statesAU); 	break;
		default:$("#billState").html(statesNotAvailable);
	}
	state = document.getElementById("newCountry").value;
	switch(state)
	{
		case "US": $("#newState").html(statesUS); 	break;
		case "CA": $("#newState").html(statesCA); 	break;
		case "AU": $("#newState").html(statesAU); 	break;
		default:$("#newState").html(statesNotAvailable);
	}	
	if (localStorage.getItem("billState") === null) {
		$("#billState").val();
	}else{
